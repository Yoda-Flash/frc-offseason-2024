...
public class PIDPivotSetpoint extends Command {
  ...
  public PIDPivotSetpoint(Pivot pivot, double setpoint) {
    m_pivot = pivot;
    m_setpoint = setpoint;
 ...
 @Override
 public void execute() {
   m_speed = m_pid.calculate(m_pivot.getEncoderPosition(), m_setpoint);

   if (!(Math.abs(m_pivot.getEncoderPosition() - m_setpoint)<= Config.kDeadband)){
     SmartDashboard.putNumber("PID value", m_speed);
     SmartDashboard.putNumber("PID Error", m_pivot.getEncoderPosition() - m_setpoint);
