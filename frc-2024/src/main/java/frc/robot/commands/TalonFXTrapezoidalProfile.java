...
/*
 * Talons in pheonix6 use rot/sec.
 * This code is all written using seconds, whereas SparkMaxTrapezoidal uses minutes.
 */
public class TalonFXTrapezoidalProfile extends Command {

  private TalonFXMotor m_motor;
  private TrapezoidProfile m_profile = new TrapezoidProfile(new TrapezoidProfile.Constraints(5.0, 10.0));
  private PIDController m_pid = new PIDController(0.15, 0, 0.0);
  private SimpleMotorFeedforward m_ff = new SimpleMotorFeedforward(0.0, 0.079552517091361);
  private State m_sample = new State();
  private State m_goal;
  private double m_prevTime;

  /** Creates a new TrapezoidProfileTest. */
  public TalonFXTrapezoidalProfile(TalonFXMotor motor, double goal) {
    m_motor = motor;
    m_goal = new State(goal, 0);
    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements(m_motor);
    m_pid.setTolerance(0.0);
  }
  ...
  public void execute() {
    ...
    m_sample = m_profile.calculate((currTime - m_prevTime), m_sample, m_goal);
    m_prevTime = currTime;
